...
def load_model(cfg_file, nc=None, export_divide_factor=None) -> Model:
    ...
    my_model = Model(cfg_file, nc=nc, export_divide_factor=export_divide_factor, is_file=not (isinstance(cfg_file, dict) or not os.path.isfile(cfg_file)))
...
class EdgeYOLO:
    ...
    def __init__(self, cfg_file=None, weights=None, rank=0, write_cfg_to_weights=False, nc=None, export_divide_factor=None):
        self.rank = rank
        ...
        if weights is not None and os.path.isfile(weights):
            ...
            self.model = load_model(self.cfg_data, nc=self.ckpt["cfg_data"]["nc"], export_divide_factor=export_divide_factor)
            try:
