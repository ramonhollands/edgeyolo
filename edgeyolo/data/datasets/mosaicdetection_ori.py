...
class MosaicDetection(Dataset):
    ...
    def __init__(
        ...
        mosaic_prob=1.0, mixup_prob=1.0, rank=0, train_mask=True, *args
    ):
        """
        Args:
    ...
    def __getitem__(self, idx):
        if self.enable_mosaic and random.random() < self.mosaic_prob:
            ...
            if (
                ...
                if self.train_mask:
                    mosaic_img, mosaic_labels, segments = self.mixup(mosaic_img, mosaic_labels, self.input_dim, segments)
                else:
            ...
            if (
                self.enable_mixup and self.enable_mosaic
                and not len(label) == 0
                and random.random() < self.mixup_prob
            ):
                if self.train_mask:
                    img, label, segms = self.mixup(img, label, self.input_dim, segms)
                else:
                    img, label = self.mixup(img, label, self.input_dim)
    ...
    def mixup(self, origin_img, origin_labels, input_dim, origin_segms=None):
        ...
        origin_img = r * origin_img + (1 - r) * padded_cropped_img.astype(np.float32)
